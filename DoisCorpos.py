...
plt.axis([-2.5,1.5,-1.5,1.5])
#plt.xticks(np.linspace(,,,endpoint=True))
#plt.yticks(np.linspace(,,,endpoint=True))
...
plt.title(r'Posi\c{c}\~{a}o Terra Sol Velocidade Inicial 2$\pi$+1', fontsize=12)
plt.grid()
plt.plot(xTerra, yTerra,'r-', linewidth=1)
plt.savefig("DC1.pdf", dpi=96)
plt.show()
